 ...
 * 缓存写入处理器
 * 
 * <p>负责将方法执行结果写入缓存，只有在条件满足且方法执行成功时才会写入
 * 
 * @author David
 */
@Slf4j
...
public class CacheWriteHandler extends Handler {
    ...
    public void handleRequest(AspectContext aspectContext) {
        ...
        if (aspectContext.getCacheHit()) {
            log.debug("缓存已命中，跳过缓存写入");
            executeHandle(aspectContext);
            return;
        }

        // 如果方法未执行或执行失败，不写入缓存
        if (!aspectContext.getMethodInvoked() || aspectContext.getMethodResult() == null) {
            log.debug("方法未执行或返回null，跳过缓存写入");
            executeHandle(aspectContext);
            return;
        }

        if (aspectContext.getMetadata() == null) {
            log.debug("缓存元数据为空，跳过缓存写入");
            executeHandle(aspectContext);
            return;
        }

        // 检查 unless 条件
        if (shouldSkipCacheWrite(aspectContext)) {
    ...
    /**
     * 检查是否应该跳过缓存写入（基于 unless 条件）
     */
    private boolean shouldSkipCacheWrite(AspectContext aspectContext) {
        ...
        try {
            ...
            log.warn("评估unless条件时出错，跳过缓存写入：{}", unless, e);
            return true;
        }
    }
}
