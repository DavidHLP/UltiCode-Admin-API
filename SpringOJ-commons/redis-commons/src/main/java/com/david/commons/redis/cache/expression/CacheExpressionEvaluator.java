 ...
 * 缓存 SpEL 表达式求值器
 * <p>
 * 负责解析和求值缓存注解中的 SpEL 表达式，包括缓存键、条件表达式等。
 * 支持引用方法参数、返回值、目标对象等上下文信息。
 * </p>
 *
 * @author David
 */
...
public class CacheExpressionEvaluator {
     ...
     * @return 条件求值结果
     */
    public boolean evaluateCondition(String conditionExpression, CacheContext context, Object result) {
        ...
        try {
            ...
            Object value = expression.getValue(evalContext, Boolean.class);
            return value != null ? (Boolean) value : false;
        } catch (Exception e) {
            log.error("Error evaluating condition expression: {}", conditionExpression, e);
    ...
    private void registerUtilityFunctions(StandardEvaluationContext evalContext) {
        ...
        java.lang.reflect.Method hasTextMethod = getMethodByName(org.springframework.util.StringUtils.class, "hasText",
                String.class);
        ...
        // 注册集合工具函数
        java.lang.reflect.Method isCollectionEmptyMethod = getMethodByName(
                org.springframework.util.CollectionUtils.class, "isEmpty", java.util.Collection.class);
        if (isCollectionEmptyMethod != null) {
            evalContext.registerFunction("isCollectionEmpty", isCollectionEmptyMethod);
        }

        // 注册对象工具函数
        java.lang.reflect.Method isNullMethod = getMethodByName(java.util.Objects.class, "isNull", Object.class);
        if (isNullMethod != null) {
            evalContext.registerFunction("isNull", isNullMethod);
        }

        java.lang.reflect.Method nonNullMethod = getMethodByName(java.util.Objects.class, "nonNull", Object.class);
     ...
     * 通过反射获取方法
     */
    private java.lang.reflect.Method getMethodByName(Class<?> clazz, String methodName, Class<?>... paramTypes) {
        try {
            return clazz.getMethod(methodName, paramTypes);
        } catch (NoSuchMethodException e) {
            log.warn("Method not found: {}.{}", clazz.getSimpleName(), methodName);
            return null;
        }
    }
    ...
    public void clearExpressionCache() {
        ...
        log.debug("Cache expression cache cleared");
    }
