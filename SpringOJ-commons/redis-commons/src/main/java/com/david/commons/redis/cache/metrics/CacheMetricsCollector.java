...
import com.david.commons.redis.cache.fallback.FallbackMetrics;
 ...
 * 缓存性能指标收集器
 * <p>
 * 收集和统计缓存操作的性能指标，包括命中率、响应时间、错误率等。
 * 支持按操作类型、方法名等维度进行统计。
 * </p>
 *
 * @author David
...
public class CacheMetricsCollector {
    ...
    // 按操作类型统计
    private final ConcurrentHashMap<CacheOperation, OperationMetrics> operationMetrics = new ConcurrentHashMap<>();
    ...
    public CacheOperationTimer startOperation(CacheMetadata metadata, String methodName) {
        ...
        // 更新操作类型指标
        operationMetrics.computeIfAbsent(metadata.getOperation(), k -> new OperationMetrics()).totalOperations
                .increment();

        // 更新方法指标
        methodMetrics.computeIfAbsent(methodName, k -> new MethodMetrics()).totalOperations.increment();
    ...
    public void recordSuccess(CacheOperationTimer timer, boolean cacheHit) {
        long duration = System.nanoTime() - timer.getStartTime();

        successfulOperations.increment();
        totalResponseTime.addAndGet(duration);

        // 记录缓存命中情况
        if (timer.getMetadata().getOperation() == CacheOperation.CACHEABLE) {
            if (cacheHit) {
                cacheHits.increment();
            } else {
        ...
        // 更新操作类型指标
        OperationMetrics opMetrics = operationMetrics.get(timer.getMetadata().getOperation());
        if (opMetrics != null) {
        ...
        // 更新方法指标
        MethodMetrics methodMetrics = this.methodMetrics.get(timer.getMethodName());
        ...
        log.debug("Cache operation completed successfully. Method: {}, Operation: {}, Duration: {}ns, CacheHit: {}",
                timer.getMethodName(), timer.getMetadata().getOperation(), duration, cacheHit);
    }
    ...
    public void recordFailure(CacheOperationTimer timer, Exception exception) {
        ...
        log.warn("Cache operation failed. Method: {}, Operation: {}, Duration: {}ns, Error: {}",
                timer.getMethodName(), timer.getMetadata().getOperation(), duration, exception.getMessage());
    }
    ...
    public CacheMetrics getGlobalMetrics() {
        ...
        return CacheMetrics.builder()
                ...
                .averageResponseTime(successful > 0 ? (double) totalTime / successful / 1_000_000 : 0.0) // 转换为毫秒
                .build();
    }
    ...
    public CacheMetrics getOperationMetrics(CacheOperation operation) {
        ...
        return CacheMetrics.builder()
                ...
                .cacheMisses(misses)
                .hitRate((hits + misses) > 0 ? (double) hits / (hits + misses) : 0.0)
                .averageResponseTime(successful > 0 ? (double) totalTime / successful / 1_000_000 : 0.0)
                .build();
    }
