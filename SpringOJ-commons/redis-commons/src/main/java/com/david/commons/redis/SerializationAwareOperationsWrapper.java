...
/**
 * 序列化感知的操作包装器 为 Redis 操作添加序列化性能监控功能
 *
 * @author David
 */
@Slf4j
public class SerializationAwareOperationsWrapper {
    ...
    public SerializationAwareOperationsWrapper(
            RedisUtilsImpl redisUtils, SerializationType serializationType) {
    ...
    /** 序列化感知的字符串操作实现 */
    private record SerializationAwareStringOperations<T>(
            RedisStringOperations<T> delegate,
            RedisUtilsImpl redisUtils,
            SerializationType serializationType)
            implements RedisStringOperations<T> {
        ...
        public Boolean set(String key, T value, long timeout, TimeUnit unit) {
            return executeWithMetrics(
                    "setWithTimeout", () -> delegate.set(key, value, timeout, unit), value);
        }

        @Override
        public Boolean set(String key, T value, Duration duration) {
            return executeWithMetrics(
                    "setWithDuration", () -> delegate.set(key, value, duration), value);
        }
        ...
        public Boolean setIfAbsent(String key, T value, long timeout, TimeUnit unit) {
            return executeWithMetrics(
                    "setIfAbsentWithTimeout",
                    () -> delegate.setIfAbsent(key, value, timeout, unit),
                    value);
        }
        ...
        public Long increment(String key, long delta) {
            return executeWithMetrics(
                    "incrementByDelta", () -> delegate.increment(key, delta), key);
        }
        ...
        public Long decrement(String key, long delta) {
            return executeWithMetrics(
                    "decrementByDelta", () -> delegate.decrement(key, delta), key);
        }
        ...
        public Long delete(String... keys) {
            return executeWithMetrics("delete", () -> delegate.delete(keys), keys);
        }
        ...
        private <R> R executeWithMetrics(String operationName, Supplier<R> operation, Object data) {
            ...
            try {
                ...
                redisUtils.recordSerializationMetrics(
                        serializationType, duration, success, dataSize);
            }
        }
    ...
    /** 序列化感知的哈希操作实现 */
    private record SerializationAwareHashOperations<T>(
            RedisHashOperations<T> delegate,
            RedisUtilsImpl redisUtils,
            SerializationType serializationType)
            implements RedisHashOperations<T> {
        ...
        public Boolean putIfAbsent(String key, String hashKey, T value) {
            return executeWithMetrics(
                    "putIfAbsent", () -> delegate.putIfAbsent(key, hashKey, value), value);
        }
        ...
        public Long increment(String key, String hashKey, long delta) {
            return executeWithMetrics(
                    "increment", () -> delegate.increment(key, hashKey, delta), key);
        }

        @Override
        public Double increment(String key, String hashKey, double delta) {
            return executeWithMetrics(
                    "incrementDouble", () -> delegate.increment(key, hashKey, delta), key);
        }
        ...
        private int estimateDataSize(Object data) {
            ...
            if (data instanceof String) return ((String) data).length() * 2;
            if (data instanceof byte[]) return ((byte[]) data).length;
            if (data instanceof Collection) return ((Collection<?>) data).size() * 10;
            if (data instanceof Map) return ((Map<?, ?>) data).size() * 20;
            return data.toString().length() * 2;
        }
    }
}
