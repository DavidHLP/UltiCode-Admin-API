...
public class CacheStorageHandler extends AbstractAspectHandler {
     ...
     * 构造函数 - 初始化缓存存储处理器
     *
     * <p>通过依赖注入方式获取所需的工具类和配置信息，用于后续的缓存存储操作。 包括日志工具、Redis操作工具、缓存条件评估器和Redis配置属性。
     *
     ...
     * 获取支持的切面类型
     *
     * <p>定义当前处理器支持处理的切面类型，用于框架识别和分发请求。 当前处理器只支持CACHE类型的切面操作。
     *
     ...
     * 获取处理器执行顺序
     *
     * <p>定义在切面链中的执行优先级，数值越大执行越晚。 设置为60确保在方法执行完成后进行缓存存储操作。
     *
     ...
     * 判断是否可以处理当前请求
     *
     * <p>根据多个条件判断当前处理器是否应该处理这个请求： 1. 首先调用父类的canHandle方法进行基础检查 2. 检查缓存是否已命中（如果已命中则无需存储） 3.
     * 检查缓存条件是否满足（通过条件评估器判断） 4. 检查方法是否已执行完成 5. 检查方法执行是否无异常
     *
     * <p>只有在缓存未命中、满足缓存条件、方法执行成功的情况下才进行缓存存储。
     *
     ...
     * 处理缓存存储逻辑
     *
     * <p>这是处理器的核心方法，负责执行实际的缓存存储操作： 1. 从上下文中获取缓存键和方法执行结果 2. 判断是否应该缓存该结果（考虑null值处理和缓存条件） 3.
     * 如果满足缓存条件，则将结果存储到Redis中 4. 记录缓存存储的操作日志 5. 即使缓存存储失败也不影响业务逻辑，继续执行后续处理器
     *
     ...
     * 判断是否应该缓存结果
     *
     * <p>通过多层条件检查来判断当前方法执行结果是否应该被缓存： 1. 获取方法上的@RedisCacheable注解 2. 检查是否允许缓存null值或结果不为null 3.
     * 使用条件评估器检查自定义缓存条件是否满足
     *
     * <p>使用Optional链式调用提高代码可读性并避免空指针异常。
     *
     ...
     * 检查是否应该缓存（考虑null值处理）
     *
     * <p>根据注解配置和方法执行结果判断是否进行缓存： - 如果结果不为null，则可以缓存 -
     * 如果结果为null，则检查注解是否允许缓存null值（cacheNullValues=true）
     *
     * <p>这样可以避免缓存不必要的null值，同时支持有意义的null结果缓存。
     *
     ...
     * 评估缓存条件
     *
     * <p>使用缓存条件评估器来检查注解中定义的自定义缓存条件是否满足。 条件表达式通常使用SpEL（Spring Expression Language）编写，
     ...
     * 缓存结果到Redis
     *
     * <p>获取方法上的@RedisCacheable注解，并执行实际的缓存存储操作。 使用Optional模式安全地获取注解信息，避免空指针异常。
     ...
     * 执行实际的缓存存储操作
     *
     * <p>根据注解配置将数据存储到Redis中： 1. 通过determineTtl方法计算TTL（生存时间） 2. 使用RedisUtils的字符串操作接口存储数据 3.
     * 如果存储过程发生异常，包装为RuntimeException重新抛出
     *
     * <p>这样设计可以让上层调用者统一处理缓存存储异常。
     *
     ...
     * 确定缓存的TTL（生存时间）
     *
     * <p>根据注解配置计算缓存数据的过期时间： - 如果注解中指定了TTL值（大于0），则使用指定的秒数 - 如果注解中未指定TTL或值为0/负数，则使用全局默认TTL配置
     *
     * <p>这种设计既支持方法级别的精细化TTL控制，也提供合理地默认值。
     *
