 ...
 * 
 * <p>
 * 实现所有分布式锁相关的操作方法，继承抽象基类以复用通用逻辑
 * 
 * @author David
 */
@Slf4j
public class RedisLockOperationsImpl extends AbstractRedisOperations implements RedisLockOperations {
     ...
     * 构造函数 - 完整版本
     */
    public RedisLockOperationsImpl(RedisTemplate<String, Object> redisTemplate,
    ...
    public RedisLock tryLock(String lockKey, Duration waitTime, Duration leaseTime) {
        return executeOperation("TRY_LOCK", lockKey, () -> {
    ...
    public RedisLock tryLock(String lockKey) {
        return executeOperation("TRY_LOCK_DEFAULT", lockKey, () -> {
    ...
    public <T> T executeWithLock(String lockKey, Duration waitTime, Duration leaseTime, Supplier<T> action) {
        return executeOperation("EXECUTE_WITH_LOCK", lockKey, () -> {
            return distributedLockManager.executeWithLock(lockKey, waitTime, leaseTime, action);
        });
    }

    @Override
    public void executeWithLock(String lockKey, Duration waitTime, Duration leaseTime, Runnable action) {
        executeOperation("EXECUTE_WITH_LOCK_VOID", lockKey, () -> {
    ...
    public <T> T executeWithLock(String lockKey, Supplier<T> action) {
        return executeOperation("EXECUTE_WITH_LOCK_DEFAULT", lockKey, () -> {
    ...
    public void executeWithLock(String lockKey, Runnable action) {
        executeOperation("EXECUTE_WITH_LOCK_DEFAULT_VOID", lockKey, () -> {
    ...
    public <T> T executeWithLockRetry(String lockKey, Duration waitTime, Duration leaseTime, Supplier<T> action) {
        return executeOperation("EXECUTE_WITH_LOCK_RETRY", lockKey, () -> {
            return distributedLockManager.executeWithLockRetry(lockKey, waitTime, leaseTime, action);
        });
    }

    @Override
    public void executeWithLockRetry(String lockKey, Duration waitTime, Duration leaseTime, Runnable action) {
        executeOperation("EXECUTE_WITH_LOCK_RETRY_VOID", lockKey, () -> {
            distributedLockManager.executeWithLockRetry(lockKey, waitTime, leaseTime, action);
            return null;
        });
    }
    ...
    public <T> T executeWithLockRetry(String lockKey, Supplier<T> action) {
        return executeOperation("EXECUTE_WITH_LOCK_RETRY_DEFAULT", lockKey, () -> {
    ...
    public void executeWithLockRetry(String lockKey, Runnable action) {
        executeOperation("EXECUTE_WITH_LOCK_RETRY_DEFAULT_VOID", lockKey, () -> {
    ...
    public <T> T executeWithLockOrFallback(String lockKey, Duration waitTime, Duration leaseTime,
        ...
        return executeOperation("EXECUTE_WITH_LOCK_OR_FALLBACK", lockKey, () -> {
            return distributedLockManager.executeWithLockOrFallback(lockKey, waitTime, leaseTime, action,
                    fallbackAction);
        });
    }
    ...
    public void executeWithLockOrFallback(String lockKey, Duration waitTime, Duration leaseTime,
        ...
        executeOperation("EXECUTE_WITH_LOCK_OR_FALLBACK_VOID", lockKey, () -> {
            distributedLockManager.executeWithLockOrFallback(lockKey, waitTime, leaseTime, action, fallbackAction);
            return null;
        });
    }

    @Override
    public <T> T executeWithLockOrFallback(String lockKey, Supplier<T> action, Supplier<T> fallbackAction) {
        return executeOperation("EXECUTE_WITH_LOCK_OR_FALLBACK_DEFAULT", lockKey, () -> {
            Duration defaultWaitTime = redisCommonsProperties.getLock().getDefaultWaitTime();
            Duration defaultLeaseTime = redisCommonsProperties.getLock().getDefaultLeaseTime();
            return distributedLockManager.executeWithLockOrFallback(lockKey, defaultWaitTime, defaultLeaseTime,
                    action, fallbackAction);
        });
    }

    @Override
    public void executeWithLockOrFallback(String lockKey, Runnable action, Runnable fallbackAction) {
        executeOperation("EXECUTE_WITH_LOCK_OR_FALLBACK_DEFAULT_VOID", lockKey, () -> {
            Duration defaultWaitTime = redisCommonsProperties.getLock().getDefaultWaitTime();
            Duration defaultLeaseTime = redisCommonsProperties.getLock().getDefaultLeaseTime();
            distributedLockManager.executeWithLockOrFallback(lockKey, defaultWaitTime, defaultLeaseTime, action,
                    fallbackAction);
            return null;
        });
    }
    ...
    public boolean isLockExists(String lockKey) {
        return executeOperation("IS_LOCK_EXISTS", lockKey, () -> {
    ...
    public boolean forceUnlock(String lockKey) {
        return executeOperation("FORCE_UNLOCK", lockKey, () -> {
    ...
    public long getRemainingTimeToLive(String lockKey) {
        return executeOperation("GET_REMAINING_TTL", lockKey, () -> {
            ...
            try {
                ...
                log.error("获取锁剩余租约时间失败: {}", fullLockKey, e);
                return -1L;
            }
        });
    }
    ...
    public boolean isHeldByCurrentThread(String lockKey) {
        return executeOperation("IS_HELD_BY_CURRENT_THREAD", lockKey, () -> {
            validateLockKey(lockKey);
            String fullLockKey = buildLockKey(lockKey);
            ...
            try {
                ...
                log.error("检查当前线程是否持有锁失败: {}", fullLockKey, e);
                return false;
            }
        });
    }
    ...
    public int getHoldCount(String lockKey) {
        return executeOperation("GET_HOLD_COUNT", lockKey, () -> {
            validateLockKey(lockKey);
            String fullLockKey = buildLockKey(lockKey);
            ...
            try {
                ...
                log.error("获取锁持有计数失败: {}", fullLockKey, e);
                return 0;
            }
        });
    }
