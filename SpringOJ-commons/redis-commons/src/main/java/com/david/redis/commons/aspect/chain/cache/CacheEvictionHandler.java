...
public class CacheEvictionHandler extends AbstractAspectHandler {
    ...
    /**
     * 判断当前处理器是否能够处理给定的切面上下文 检查是否满足以下条件： 1. 父类canHandle返回true 2. 存在RedisEvict注解 3. 条件表达式满足 4.
     * 驱逐操作尚未处理过
     *
    ...
    /**
     * 处理缓存驱逐逻辑的主入口方法 根据注解的beforeInvocation属性决定是在方法执行前还是执行后进行缓存驱逐
     *
    ...
    public Object handle(AspectContext context, AspectChain chain) throws Throwable {
        var annotation =
                getRedisEvictAnnotation(context)
                        .orElseThrow(
                                () -> new IllegalStateException("RedisEvict annotation not found"));
    ...
    /**
     * 处理方法执行前的缓存驱逐操作 先执行缓存驱逐，然后标记为已处理，最后继续执行切面链
     *
     * @param context 切面执行上下文
     * @param chain 切面处理链
     ...
     */
    private Object handleBeforeInvocation(
            AspectContext context, AspectChain chain, String evictProcessedKey) throws Throwable {
        evictCache(context);
    ...
    /**
     * 处理方法执行后的缓存驱逐操作 先执行切面链中的方法，如果方法执行成功则进行缓存驱逐并标记为已处理
     *
     * @param context 切面执行上下文
     * @param chain 切面处理链
     * @param evictProcessedKey 用于标记驱逐操作已处理的键
     ...
     * @throws Throwable 可能抛出的异常
     */
    private Object handleAfterInvocation(
            AspectContext context, AspectChain chain, String evictProcessedKey) throws Throwable {
    ...
    /**
     * 处理缓存驱逐过程中发生的异常 记录异常日志，如果是方法执行前的驱逐失败，标记为已处理以避免重复处理
     *
     * @param context 切面执行上下文
     * @param chain 切面处理链
     * @param e 发生的异常
     ...
     */
    private Object handleEvictionException(
            AspectContext context,
            AspectChain chain,
            Exception e,
            boolean beforeInvocation,
            String evictProcessedKey)
            throws Throwable {
    ...
    /**
     * 执行缓存驱逐操作的核心方法 如果是虚拟上下文（method为null），则跳过驱逐操作 根据RedisEvict注解配置执行相应的驱逐策略
     *
     * @param context 切面执行上下文
     */
    ...
    /**
     * 根据驱逐时机配置处理不同类型的缓存驱逐策略 支持延迟驱逐、级联驱逐和立即驱逐三种模式
     *
     * @param context 切面执行上下文
    ...
    /**
     * 执行立即缓存驱逐操作 根据softDelete配置决定是执行软删除还是硬删除
     *
     * @param context 切面执行上下文
    ...
    /**
     * 执行硬删除操作，直接从Redis中删除缓存数据 根据allEntries配置决定是删除所有匹配条目还是指定键
     *
     * @param context 切面执行上下文
    ...
    /**
     * 执行软删除操作，不直接删除数据而是重命名键添加删除标记 软删除的数据可以在需要时恢复，适用于需要保留数据备份的场景
     *
     * @param context 切面执行上下文
     * @param annotation RedisEvict注解配置
     */
    ...
    /**
     * 对所有匹配的缓存条目执行软删除操作 使用模式匹配找到所有相关的键，然后逐个进行软删除处理
     *
     * @param context 切面执行上下文
     * @param annotation RedisEvict注解配置
    ...
    /**
     * 对指定的缓存键执行软删除操作 根据keys配置生成要软删除的键列表，然后逐个处理
     *
     * @param context 切面执行上下文
     * @param annotation RedisEvict注解配置
     * @param suffix 软删除后缀标识
     */
    private void performSoftDeleteSpecific(
            AspectContext context, RedisEvict annotation, String suffix) {
        List<String> keysToSoftDelete = generateKeysToEvict(context, annotation, annotation.keys());

        int softDeleteCount =
                keysToSoftDelete.stream()
                        .filter(Predicate.not(this::isDeletedKey))
                        .mapToInt(key -> processSoftDeleteKey(key, suffix) ? 1 : 0)
                        .sum();
    ...
    /**
     * 判断给定的键是否为已被软删除的键 通过检查键名是否包含删除标记后缀来判断
     *
     * @param key 要检查的键名
    ...
    /**
     * 调度延迟执行的缓存驱逐操作 使用CompletableFuture的延迟执行器在指定时间后异步执行驱逐操作
     *
     * @param context 切面执行上下文
     * @param annotation RedisEvict注解配置
    ...
    private void scheduleDelayedEviction(
            AspectContext context, RedisEvict annotation, long delayMs) {
        if (delayMs <= 0) {
            performImmediateEviction(context, annotation);
            return;
        }
    ...
    /**
     * 执行级联缓存驱逐操作 先执行基本的驱逐操作，然后根据cascadePatterns配置执行额外的级联删除 适用于需要同时删除相关联缓存数据的场景
     *
     * @param context 切面执行上下文
    ...
    /**
     * 处理单个级联模式的缓存删除 解析SpEL表达式生成具体的模式，然后执行模式匹配删除
     *
     * @param context 切面执行上下文
    ...
    private void processCascadePattern(AspectContext context, String pattern, String keyPrefix) {
        try {
            String resolvedPattern =
                    keyGenerator.resolveSpELExpression(
    ...
    /**
     * 驱逐所有匹配指定模式的缓存条目 使用通配符模式找到所有相关的键，然后批量删除
     *
     * @param context 切面执行上下文
     * @param annotation RedisEvict注解配置
     */
    ...
    /**
     * 驱逐注解中指定的特定缓存键 根据keys配置生成具体的键列表，然后批量删除这些键
     *
     * @param context 切面执行上下文
    ...
    /**
     * 根据键表达式生成要驱逐的具体键列表 遍历所有键表达式，使用SpEL解析器生成实际的缓存键
     *
     * @param context 切面执行上下文
     * @param annotation RedisEvict注解配置
    ...
    /**
     * 生成单个缓存键 使用CacheKeyGenerator解析键表达式，处理其中的SpEL语法和参数替换
     *
     * @param context 切面执行上下文
     ...
     */
    private Optional<String> generateSingleKey(
            AspectContext context, String keyExpression, String keyPrefix) {
        try {
            String generatedKey =
                    keyGenerator.generateKey(keyExpression, context.getMethod(), context.getArgs());
    ...
    /**
     * 执行实际的缓存驱逐删除操作 将键列表按精确键和模式键分组，分别进行批量删除和模式匹配删除
     *
     * @param context 切面执行上下文
    ...
    private int executeEviction(AspectContext context, List<String> keysToEvict) {
        var keyGroups = keysToEvict.stream().collect(Collectors.groupingBy(this::isPatternKey));
        ...
        totalDeleted +=
                patternKeys.stream()
                        .mapToInt(
                                pattern ->
                                        Optional.ofNullable(deleteByPattern(context, pattern))
                                                .orElse(0L)
                                                .intValue())
                        .sum();
    ...
    /**
     * 判断给定的键是否为模式键（包含通配符） 检查键中是否包含*或?等通配符字符
     *
     * @param key 要检查的键
    ...
    /**
     * 根据模式删除匹配的缓存键 使用模式匹配找到所有符合条件的键，然后批量删除
     *
     * @param context 切面执行上下文
    ...
    /**
     * 检查驱逐条件是否满足 从上下文中获取条件处理器设置的条件满足标记
     *
     * @param context 切面执行上下文
     * @return true表示条件满足，false表示条件不满足
     */
    private boolean isConditionMet(AspectContext context) {
        return context.getAttribute(CacheConditionHandler.EVICT_CONDITION_MET_ATTR, Boolean.class,true);
    }

    /**
     * 检查驱逐操作是否已经处理过 根据beforeInvocation属性检查相应的处理标记，避免重复执行驱逐操作
     *
     * @param context 切面执行上下文
    ...
    private boolean isEvictProcessed(AspectContext context) {
        return getRedisEvictAnnotation(context)
                .map(
                        annotation -> {
                            String evictProcessedKey =
                                    EVICT_PROCESSED_PREFIX + annotation.beforeInvocation();
                            return context.getAttribute(evictProcessedKey, Boolean.class, false);
                        })
                .orElse(false);
    }

    /**
     * 根据模式获取匹配的Redis键集合 优先使用scan命令避免阻塞，如果scan结果为空则回退到keys命令
     *
     * @param pattern 匹配模式
     * @return 匹配的键集合
     */
    ...
    /**
     * 格式化键列表用于日志输出 当键数量较少时显示完整列表，数量较多时只显示前几个并标注总数
     *
     * @param keys 要格式化的键集合
     * @return 格式化后的字符串表示
     */
    private String formatKeysList(Set<String> keys) {
        if (keys.size() <= MAX_DISPLAY_KEYS) {
            ...
            String preview =
                    keys.stream().limit(PREVIEW_KEYS_COUNT).collect(Collectors.joining(", "));
            return "[%d个key: %s...]".formatted(keys.size(), preview);
        }
    }

    /**
     * 获取缓存键前缀 优先使用注解中指定的keyPrefix，如果未设置则使用全局配置的前缀
     *
