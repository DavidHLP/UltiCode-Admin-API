...
/** Java 判题处理器集成测试 测试完整的 JSON 格式判题流程：格式化 -> 编译 -> 运行 -> 比较 */
@SpringBootTest(classes = SandBoxSpringApplication.class)
public class JavaHandlersTest {

    @Autowired private JavaFormatCodeHandler formatHandler;

    @Autowired private JavaCompileHandler compileHandler;

    @Autowired private JavaRunHandler runHandler;

    @Autowired private JavaCompareHandler compareHandler;

    /** 在每个用例前，按责任链顺序串联处理器： format -> compile -> run -> compare */
    @BeforeEach
    ...
    public void testCompleteJudgeFlow_CorrectAnswer() {
        ...
        // 一键链式执行：从 formatHandler 作为起点触发全链路
        Boolean chainResult = formatHandler.handleRequest(context);
        assertTrue(chainResult, "责任链应执行成功");
        ...
        for (JudgmentResult result : context.getJudgmentResults()) {
            assertEquals(JudgeStatus.ACCEPTED, result.getJudgeStatus(), "每个测试用例都应该通过");
        }
    }
    ...
    public void testCompleteJudgeFlow_WrongAnswer() {
        ...
        // 一键链式执行
        Boolean chainResult = formatHandler.handleRequest(context);
        assertTrue(chainResult, "责任链应完成执行（最终比较返回 true）");

        // 验证结果
        assertEquals(JudgeStatus.WRONG_ANSWER, context.getJudgeStatus(), "错误答案应该得到 WRONG_ANSWER");
    }
    ...
    public void testCompleteJudgeFlow_CompileError() {
        ...
        // 一键链式执行（在编译环节即失败，整体返回 false）
        Boolean chainResult = formatHandler.handleRequest(context);

        // 验证结果
        assertFalse(chainResult, "编译错误的代码应该导致责任链返回 false");
        assertEquals(JudgeStatus.COMPILE_ERROR, context.getJudgeStatus(), "应该得到 COMPILE_ERROR 状态");
