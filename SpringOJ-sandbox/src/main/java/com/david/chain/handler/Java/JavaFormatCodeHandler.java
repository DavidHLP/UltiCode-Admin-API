 ...
 * Java 代码格式化处理器
 *
 * <p>功能： 1. 基于新的 JSON 系统生成完整的可运行代码 2. 将 Solution 类与 Main 类合并 3. 严格验证所有输入输出为 JSON 格式
 */
@Slf4j
@Component
...
public class JavaFormatCodeHandler extends Handler {
    ...
    public Boolean handleRequest(JudgmentContext judgmentContext) {
        try {
            ...
            String completeCode =
                    javaCodeUtils.generateMainFixSolutionClass(mainClass, solutionClass);
    ...
    private boolean validateContext(JudgmentContext context) {
        ...
        if (context.getSolutionFunctionName() == null
                || context.getSolutionFunctionName().trim().isEmpty()) {
    ...
    private String generateMainClass(JudgmentContext context) {
        ...
        List<List<String>> allInputTypes =
                testCases.stream()
                        .map(
                                tc ->
                                        tc.getTestCaseInput().stream()
                                                .map(TestCaseInput::getInputType)
    ...
    private boolean isValidJsonFormat(String input) {
	    ...
	    return input.equals("null"); // null值
    }
    ...
    private String wrapUserCodeInSolutionClass(JudgmentContext context) {
        ...
        // 检查是否需要 List 相关的导入
        boolean needsUtilImport =
                userCode.contains("List<")
