 ...
 * Java 代码编译处理器
 *
 * <p>功能： 1. 将格式化后的代码写入临时文件 2. 调用 javac 进行编译 3. 捕获编译错误信息 4. 管理编译超时
 */
@Slf4j
@Component
...
public class JavaCompileHandler extends Handler {
    ...
    private boolean handleCompileResult(CompileResult result, JudgmentContext context) {
        if (result.success()) {
            ...
            log.info("编译失败，提交ID: {}, 错误信息: {}", context.getSubmissionId(), result.output());

            context.setJudgeStatus(JudgeStatus.COMPILE_ERROR);
            context.setCompileInfo(formatCompileError(result.output()));
            return false;
        }
    }
    ...
    private String formatCompileError(String rawError) {
        ...
        String cleaned = rawError.replaceAll("/tmp/[^/]+/", "").replaceAll("\\\\[^\\\\]+\\\\", "");
	...
	/** 编译结果封装类 */
	    private record CompileResult(boolean success, int exitCode, String output) {
	}
}
