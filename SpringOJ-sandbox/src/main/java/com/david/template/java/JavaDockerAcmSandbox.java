...
public class JavaDockerAcmSandbox extends SandboxTemplate {
	...
	protected String createContainer(String tempDir, SandboxExecuteRequest request) {
		...
		HostConfig hostConfig = HostConfig.newHostConfig().withNetworkMode("none") // 禁止网络访问
		...
		CreateContainerResponse container = dockerClient.createContainerCmd(imageName).withHostConfig(hostConfig)
	...
	protected void startContainer(String containerId) {
		...
		// 确保/app目录有正确的权限
		try {
			ExecCreateCmdResponse chmodCmd = dockerClient.execCreateCmd(containerId).withCmd("chmod", "755", "/app")
					.exec();
	...
	protected JudgeResult compileCode(String containerId, String sourceFile, LanguageType language,
		...
		try {
			...
			ExecCreateCmdResponse execCreateCmd = dockerClient.execCreateCmd(containerId).withCmd(compileCmd)
			...
			dockerClient.execStartCmd(execCreateCmd.getId()).exec(new ResultCallback.Adapter<Frame>() {
				@Override
	...
	protected void cleanupTempDirectory(String tempDir) {
		try {
			Files.walk(Paths.get(tempDir)).sorted(java.util.Comparator.reverseOrder()).map(java.nio.file.Path::toFile)
					.forEach(File::delete);
	...
	private TestCaseResult runSingleTestCase(String containerId, String input, String expectedOutput,
		...
		try {
			...
			ExecCreateCmdResponse execCreateCmd = dockerClient.execCreateCmd(containerId).withCmd(executeCmd)
			...
			ResultCallback.Adapter<Frame> callback = dockerClient.execStartCmd(execCreateCmd.getId())
					.withStdIn(runInput).exec(new ResultCallback.Adapter<Frame>() {
						@Override
						public void onNext(Frame frame) {
							try {
								if (frame.getStreamType() == StreamType.STDOUT) {
			...
			String stderr = error.toString(StandardCharsets.UTF_8).trim();
			if (!stderr.isEmpty()) {
			...
			// Compare output
			String actualOutput = output.toString(StandardCharsets.UTF_8).replaceAll("[\\p{C}\\p{Z}]", "");
			String expectedOutputTrimmed = expectedOutput.replaceAll("[\\p{C}\\p{Z}]", "");

			if (actualOutput.equals(expectedOutputTrimmed)) {
				result.setStatus(JudgeStatus.ACCEPTED);
				result.setScore(10);// 假设每个测试用例10分
			} else {
				result.setStatus(JudgeStatus.WRONG_ANSWER);
				result.setErrorMessage("输出不匹配. 期望: " + expectedOutputTrimmed + ", 实际: " + actualOutput);
				log.error("输出不匹配. 期望: {}, 实际: {}", expectedOutputTrimmed, actualOutput);
			}
