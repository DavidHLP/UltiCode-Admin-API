...
public class RoleServiceImpl extends ServiceImpl<RoleMapper, Role> implements IRoleService {
    ...
    @Override
    @RedisCacheable(key = "'pageRoles:' + #page + ':' + #size + ':' + (#keyword != null ? #keyword : '') + ':' + (#status != null ? #status : '')", ttl = 1800, // 30分钟缓存
            type = Page.class, cacheNames = "role")
    ...
    @Override
    @RedisCacheable(key = "'list'", ttl = 1800, // 30分钟缓存
            type = List.class, cacheNames = "role")
    public List<Role> list() {
        return roleMapper.selectList(null);
    }

    @Override
    @RedisCacheable(key = "'getById:' + #id", ttl = 1800, // 30分钟缓存
            type = Role.class, cacheNames = "role")
