...
public class SolutionCommentViewController extends BaseController {
    ...
    @PostMapping
    public ResponseResult<Void> createComment(
            @RequestBody @Validated(SolutionComments.Create.class)
                    SolutionComments solutionComments) {
        if (!Objects.equals(solutionComments.getUserId(), getCurrentUserId())) {
            throw new RuntimeException("评论用户ID与当前登录用户ID不匹配");
        }
    ...
    @PutMapping
    public ResponseResult<Void> updateComment(
            @RequestBody @Validated(SolutionComments.Update.class)
                    SolutionComments solutionComments) {
    ...
    public ResponseResult<Void> deleteComment(
            @RequestBody @Validated(SolutionComments.Delete.class)
                    SolutionComments solutionComments) {
