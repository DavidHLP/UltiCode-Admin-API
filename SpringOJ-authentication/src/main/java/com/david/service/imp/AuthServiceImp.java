...
public class AuthServiceImp implements AuthService {
	...
	// Redis缓存键前缀常量
	private static final String CACHE_KEY_VERIFICATION_PREFIX = "auth:verification:";
	private static final String LOCK_KEY_LOGIN_PREFIX = "auth:lock:login:";
	private static final String LOCK_KEY_REGISTER_PREFIX = "auth:lock:register:";
	private static final String LOCK_KEY_DISTRIBUTED_LOCK_PREFIX = "auth:verification:code:";
	...
	public Token login(String email, String password) {
		AuthUser user = userMapper.loadUserByEmail(email);
		if (user == null) {
			log.error("用户不存在");
		...
		Token token = Token.builder().userId(user.getUserId()).token(accessToken).tokenType(TokenType.ACCESS).build();
		// 使用分布式锁防止并发重复写入
		executeWithLock(LOCK_KEY_LOGIN_PREFIX + email, () -> tokenMapper.insert(token));
		return token;
	}
	...
	public void sendVerificationCode(String email) {
		...
		executeWithLock(LOCK_KEY_DISTRIBUTED_LOCK_PREFIX + email, () -> redisTemplate.opsForValue().set(codeKey, code, Duration.ofMinutes(5)));
	...
	public void register(String username, String password, String email, String code) {
		...
		if (userMapper.loadUserByEmail(email) != null) {
			throw BizException.of(ResponseCode.BUSINESS_ERROR.getCode(), "用户名已存在");
		}

		AuthUser user = AuthUser.builder().username(username).password(passwordEncoder.encode(password)).email(email).status(1).build();
		// 使用分布式锁防止并发重复注册
		executeWithLock(LOCK_KEY_REGISTER_PREFIX + email, () -> userMapper.insert(user));
		// 注册完成后删除验证码
	...
	@Override
	@Transactional
	@RedisCacheable(key = "'user:token:' + #token", ttl = 1800, // 30分钟缓存
			type = AuthUser.class, cacheNames = "auth")
	public AuthUser validateToken(String token) {
		try {
			...
			AuthUser userDetails = userMapper.loadUserByEmail(email);
			if (userDetails == null) {
	...
	@Override
	@RedisCaching(redisCacheEvict = {@RedisCacheEvict(key = "'user:token:' + #token", cacheNames = "auth"), @RedisCacheEvict(key = "'user:info:' + #email", cacheNames = "auth")})
	public void logout(String email, String token) {
		tokenMapper.deleteByToken(token);
	}

	@Override
	@RedisCacheable(key = "'user:info:' + #email", ttl = 1800, // 30分钟缓存
			type = AuthUser.class, cacheNames = "auth")
	public AuthUser getUserInfo(String email) {
