...
public class AuthGlobalFilter implements GlobalFilter, Ordered {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        // 记录网关请求
        logUtils.business().audit("GATEWAY_REQUEST", "INBOUND",
                String.format("Incoming request: %s %s", method, path), requestContext);
        ...
        if (PUBLIC_PATHS.stream().anyMatch(path::startsWith)) {
            // 记录公共路径访问
            logUtils.business().audit("GATEWAY_PUBLIC_ACCESS", "ALLOWED",
                    String.format("Public path accessed: %s %s", method, path), requestContext);
            log.debug("公开路径，直接放行: {}", path);
            return chain.filter(exchange);
        }

        String authHeader = request.getHeaders().getFirst(HEADER_AUTHORIZATION);
        if (authHeader == null || authHeader.trim().isEmpty() || !authHeader.startsWith(BEARER_PREFIX)) {
            String errorMsg = "请求缺少有效的 Authorization 头";
            logUtils.security().threat("UNAUTHORIZED_ACCESS", "HIGH",
        ...
        return Mono.fromCallable(() -> {
            try {
                ...
                // 记录认证服务调用性能
                logUtils.performance().timing("AUTH_SERVICE_CALL", duration,
                        Map.of("path", path, "method", method));

                return result;
            } catch (Exception e) {
                String errorMsg = "认证服务调用异常: " + e.getMessage();
                logUtils.security().error("AUTH_SERVICE_ERROR", e,
                ...
                .flatMap(authUser -> {
                    if (authUser == null) {
                        ...
                        logUtils.security().threat("AUTHENTICATION_FAILURE", "HIGH",
                                String.format("Authentication failed: %s", errorMsg),
                                clientIp);
                        log.warn("认证服务返回空结果");
                        return handleUnauthorized(exchange.getResponse(), errorMsg);
                    }
                    ...
                    logUtils.security().login(authUser.getUsername(), "JWT", true,
                            clientIp, request.getHeaders().getFirst("User-Agent"));
